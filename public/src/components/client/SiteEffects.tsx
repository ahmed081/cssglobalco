...
export function SiteEffects() {
  ...
  useEffect(() => {
    ...
    const openBtn = document.querySelector<HTMLButtonElement>(".mob-btn");
    const closeBtn = document.getElementById("mob-close");
    const openMenu = () => { overlay?.classList.add("is-open"); document.body.classList.add("menu-open"); openBtn?.setAttribute("aria-expanded", "true"); };
    const closeMenu = () => { overlay?.classList.remove("is-open"); document.body.classList.remove("menu-open"); openBtn?.setAttribute("aria-expanded", "false"); };
    ...
    document.querySelectorAll<HTMLFormElement>("form").forEach((form) => {
      const handler = async (event: Event) => {
        ...
        if (btn) { btn.textContent = "Sending..."; btn.disabled = true; btn.style.opacity = ".72"; }
        await new Promise((resolve) => setTimeout(resolve, 450));
        if (msg) { msg.textContent = "Got it — we'll be in touch within 24 hours."; msg.style.color = "#1a6e42"; }
        form.reset();
        document.querySelectorAll(".time-slot").forEach((b) => b.classList.remove("selected"));
    ...
    const vsSwitcher = switchPanel("vs-", ".vs-panel", "#vs-tabs");
    const roleSwitcher = switchPanel("role-", ".role-panel", ".tier-tabs");
    (window as any).switchVS = vsSwitcher;
    (window as any).switchVs = vsSwitcher;
    (window as any).switchRole = roleSwitcher;
    ...
    (window as any).calcSavings = calcSavings;
    calcSavings();
    ...
    return () => {
      ...
      overlay?.querySelectorAll("a").forEach((a) => a.removeEventListener("click", closeMenu));
      observer.disconnect();
